...
class Filp10Normal extends StatefulWidget {
  @override
  _CardFlipDemoState createState() => _CardFlipDemoState();
}

class _CardFlipDemoState extends State<Filp10Normal>
  ...
  Future<void> _fetchBreakfastMenus() async {
    ...
    final List<Map<String, dynamic>> maps1 =
        await _dbHelper.getfoodbytype2('เส้น');

    setState(() {
      List<Map<String, dynamic>> allMenus = maps + maps1;
      if (maps.length >= 1) {
        _randomMenus = _generateRandomMenus(allMenus);
      } else {
        _randomMenus = allMenus;
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: Color.fromARGB(255, 157, 172, 96),
      ),
  ...
  Widget _buildTextItem(int index) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Color(0xFFFEFAE0), // ใช้โทนสีส้มอ่อน
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              ...
              color: Color.fromARGB(255, 157, 172, 96),
            ),
